...
func (c *productController) Insert(context *gin.Context) {
	...
	imageName := helper.FileUpload(context, "public/")
	productCreateDTO.Image = imageName
	productCreateDTO.WarehouseID = context.Param("warehouse_id")
	context.ShouldBind(&productCreateDTO)
...
func (c *productController) Update(context *gin.Context) {
	...
	context.ShouldBind(&productUpdateDTO)
	id := context.Param("id")
	var product model.Product = c.productService.FindByID(id)
	if (product == model.Product{}) {
		res := helper.BuildErrorResponse("Data not found", "No data with given id", helper.EmptyObj{})
		...
		if context.Param("image") != "" {
			...
			productUpdateDTO.Image = product.Image
		}
